...
class ChatManager:
    ...
    async def _generate_response(
        ...
        full_response = ""
        try:
            response_or_gen = await analyze_code(
                prompt=full_prompt,
                code="",
                language="text",
                stream=True,
            ...
            if isinstance(response_or_gen, str):
                full_response = response_or_gen
                yield response_or_gen
            else:
                async for chunk in response_or_gen:
                    full_response += chunk
                    yield chunk
        except Exception as e:
